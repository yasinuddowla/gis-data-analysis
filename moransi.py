...
if df[variable].isnull().sum() > 0 or df[['X', 'Y']].isnull().sum().sum() > 0:
    print("Warning: Missing values detected. Dropping rows with NaN.")
    df = df.dropna(subset=[variable, 'X', 'Y'])
...
plt.scatter(gdf[variable], libpysal.weights.lag_spatial(w, gdf[variable]), 
            alpha=0.5, s=50)
